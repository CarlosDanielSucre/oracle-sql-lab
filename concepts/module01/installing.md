...
Extract (if needed): If you downloaded a .zip or tarball, extract it. For .rpm, it's usually directly installable.

Install Prerequisites (Example for CentOS/RHEL):
...
Configure the Database: After installation, you'll need to run a configuration script.

sudo /etc/init.d/oracle-xe-21c configure

During configuration, you will be prompted to set the Oracle Listener port (default 1521), HTTP port (default 5500), and the SYS, SYSTEM, and PDBADMIN user passwords. Remember these passwords!

You might also be asked for the database file location and character set.

3. Verify Oracle XE Installation
Windows: Open services.msc (search in Start Menu). Look for services starting with Oracle (e.g., OracleServiceXE, OracleOraDB21Home1TNSListener). Ensure they are running.
...
You can also try accessing the Oracle SQL Developer Web interface in your browser: https://localhost:5500/em (replace localhost with your server's IP if on a remote machine).
...
Go to the official Oracle SQL Developer download page:

Oracle SQL Developer Downloads

Choose the version suitable for your OS (e.g., "Windows 64-bit with JDK 11 included" if you don't have Java installed).

You will need to accept the license agreement to download.

2. Installation/Setup
Oracle SQL Developer typically doesn't require a traditional installation.

Extract the Downloaded File: Unzip the downloaded file (e.g., sqldeveloper-21.x.x.zip) to a location on your computer (e.g., C:\sqldeveloper on Windows, or ~/sqldeveloper on Linux).
...
In SQL Developer, in the Connections panel on the left, right-click and select New Connection....
...
You are now connected to your Oracle XE database!
...
Go to the official DBeaver Community Edition download page:

DBeaver CE Downloads

Choose the installer for your operating system (Windows Installer, Linux .deb/.rpm, macOS).

2. Installation Steps
For Windows:
Run the Installer: Double-click the downloaded .exe file.

Follow the Installation Wizard: Accept the license agreement, choose installation type (e.g., "Anyone who uses this computer"), select components, and choose the installation location. Click Next and Install until finished.
...
Linux (.rpm for CentOS/RHEL/Fedora):

sudo rpm -ivh dbeaver-ce-x.x.x.rpm

macOS: Drag the DBeaver icon to your Applications folder.

3. Connect to Oracle XE
Launch DBeaver.

Click Database > New Database Connection (or the plug icon in the toolbar).

In the "Connect to a database" wizard:
...
Test a Basic Query: In your client tool, open a new SQL editor and run a simple query, for example:
